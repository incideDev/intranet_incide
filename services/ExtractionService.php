...
/**
 * Servizio per le estrazioni AI dalle gare (estratto da GareService).
...
class ExtractionService
{
    ...
    public static function listJobsByGara(int $jobId): array
    {
        ...
        \Services\GareService::ensureGaraColumnsAvailable();
    ...
    public static function getEstrazioniGara(int $jobId): array
    {
        ...
        foreach ($rows as $row) {
            ...
            // Se ci sono dati normalizzati, usali per costruire la risposta
    ...
    public static function jobPull(int $jobId): array
    {
        ...
        try {
            $timeStart = microtime(true);
            $normalizeResult = \Services\GareService::normalizeGara($jobId);
    ...
    private static function updateJobStatus(int $jobId, string $status, ?string $error = null, ?array $progress = null): void
    {
        ...
        \Services\GareService::ensureGaraColumns($pdo);
     ...
     * Converts API v1 format (results[].data) into the format expected by upsertExtractions().
     */
    ...
    public static function externalBatchStatus(string $batchId, array $env): array
    {
        $client = new \Services\AIextraction\ExternalApiClient($env);
